...
function MousePreCreate(itemClassName) {
    if (!isMouseInCellMap) {
        return;
    }
    if (iMouse == null || jMouse == null) {
        return;
            ...
            // console.log(document.querySelectorAll(`.${itemClassName}`));
...
class Cells {
    ...
    ShowColors() {
        ...
        for (let i = 0/* , color */; i < this.vNumb; ++i) {
            for (let j = 0; j < this.hNumb; ++j) {
    ...
    InitColors(width, height, size) {
                    ...
                    // console.log(width == null, height == null, size == null);
        ...
        // playgroundContainer = document.getElemthis");
    ...
    UpdateColors() {
        if (intervalID ?? false) {
            ...
            intervalID = setInterval(this.Involve.bind(this), intervalElapse);
        }
    }
}
// var newCellMap;
...
function ChangeElapse() {
    ...
    clearInterval(intervalID);
    intervalID = setInterval(playground.Involve.bind(playground), intervalElapse);
}
...
playground = new Cells(document.body, "playground");
